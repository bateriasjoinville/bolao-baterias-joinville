...
export function semanaJaIniciou(semana: SemanaIndex, now: Date = new Date()): boolean {
  const j = JANELAS.find((x) => x.semana === semana);
  if (!j) return false;
  return now.getTime() >= new Date(j.inicio).getTime();
}
